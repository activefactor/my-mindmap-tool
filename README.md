...
| サブツリーを折りたたむ / 展開 | ノード右端の小さな丸をクリック（子ノードが2つ以上のとき表示。折りたたみ時は白抜きの丸になる） |
| テキストをコピー | `Ctrl+C` / `Cmd+C`（子孫ノードも含めてテキストコピー） |
| テキストをカット | `Ctrl+X` / `Cmd+X`（コピー後にノードを削除） |
| テキストを貼り付け | `Ctrl+V` / `Cmd+V`（クリップボードのテキストを選択ノードの子として追加） |
...
| `Ctrl+X` / `Cmd+X` | 選択ノードをカット（コピー＋削除） |
| `Ctrl+V` / `Cmd+V` | クリップボードのテキストを選択ノードの子として貼り付け |
| `F` | 全体表示（フィットビュー） |
